function [NetoutValidation,test_error_Y] = bls_train_noTest_matrixTestInput(train_x,train_y,...
    validation_x,WF,WeightEnhan,s,C,NumFea,NumWin,Vp_true,den_true,f,...
    modes_num_vec,index_vec,index_vec_all,mean_y,std_y,validation_x_norm)
%   a matrix of multiple validation samples.
%
%   ** Differences from baseline (_noTest) **
%     1. validation_x_norm is (M x P): all M samples are propagated through
%        the BLS in a single matrix-vector pass ！ no loop over samples.
%     2. Rescaling: NetoutValidation is kept as a matrix (M x Q);
%        baseline flattens to a row vector with (:)' before rescaling.
%     3. Error metric: test_error_Y is hard-coded to 0 ！ forward-modelling
%        of M samples inside the grid search is too expensive, so the error
%        criterion is intentionally disabled here.
...
%% Output weights (ridge regression)
WeightTop = (T3'  *  T3+eye(size(T3',1)) * (C)) \ ( T3'  *  train_y);
NetoutTrain = T3 * WeightTop;
clear T3;

%% Validation ！ matrix input (M samples processed simultaneously)
test_x = validation_x_norm; % (M x P)
...
%% Rescale prediction to physical units
% ** Difference from baseline **: no (:)' flatten; NetoutValidation stays
% as (M x Q) matrix so all samples are rescaled column-wise in one pass.
...
%% Validation error ！ disabled for matrix input
% ** Difference from baseline **: forward-modelling M samples per grid-
% search trial is prohibitively expensive, so test_error_Y is set to 0.
% Hyperparameter selection in the wrapper therefore relies on the last
% trial's NetoutValidation rather than a meaningful error criterion.
test_error_Y=0;
