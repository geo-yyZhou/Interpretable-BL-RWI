function [NetoutValidation,test_error_Y] = bls_train_noTest(train_x,train_y,...
...
%   validation sample (baseline version ¡ª no importance output).
%
%   ** Baseline version **
%   Validation input  : validation_x_norm  (1 x P normalised row vector)
%   Error metric      : dispersion-domain RMSE via calDispersions_2
%   Output            : rescaled earth-model prediction only
%   Compare with:
%     _importance         -- also returns importance_vector
%     _matrixTestInput    -- validation_x_norm is a matrix (multiple samples)
%
...
%% Feature layer: sparse BLS mapping
H1 = [train_x,  0.1 * ones(size(train_x,1),1)];
y=zeros(size(train_x,1),NumWin*NumFea);
for i=1:NumWin
    WeightFea=WF{i};
    A1 = H1 * WeightFea;
    A1 = mapminmax(A1);%Map matrix row minimum and maximum values to [-1 1].
    clear WeightFea;
...
%% Validation - single normalised sample (baseline: row vector input)
...
%% Rescale prediction from normalised space to physical units
...
%% Validation error: dispersion-domain RMSE
invertedDispersions = calSynDispersion(Vs_inverted_profile,Vp_true,...
    den_true,f,modes_num_vec,index_vec_all);
test_error_Y = sqrt(sum((invertedDispersions-validation_x).^2/...
    length(invertedDispersions)));
