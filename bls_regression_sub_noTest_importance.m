function [Y_hat,all_index,NumFea_hat,NumWin_hat,NumEnhan_hat] = ...
    bls_regression_sub_noTest_importance(train_x,train_y,Fea_vec,Win_vec,...
    Enhan_vec,validation_x,Vp_true,den_true,f,modes_num_vec,index_vec,index_vec_all,mean_y,std_y,validation_x_norm,idx_importaceTop)
%   Grid-search wrapper for BLS hyperparameter tuning (importance version).
%   Identical to bls_regression_sub_noTest, except:
%
%   ** Difference from baseline (_noTest) **
%     Extra input : idx_importaceTop -- index vector selecting the most
%                   important frequency-mode points for error evaluation.
%                   The validation RMSE is computed only over these points
%                   rather than the full dispersion vector.
%
% Inputs: (same as baseline, plus)
%   idx_importaceTop - Index vector of high-importance frequency points
%                      used to compute the weighted validation error.
%
...
for Num_i= 1:1:length(Fea_vec) %e.g.,1:20
    for Num_j=1:1:length(Win_vec) %e.g.,1:30
        for Num_k=1:1:length(Enhan_vec) %e.g.,1:40
            ...
            % Train and evaluate ¡ª passes idx_importaceTop for importance-weighted error
            [NetoutValidation,test_error_Y] = bls_train_noTest_importance(train_x,train_y,...
                validation_x,WF,WeightEnhan,s,C,NumFea,NumWin,Vp_true,...
                den_true,f,modes_num_vec,index_vec,index_vec_all,mean_y,...
                std_y,validation_x_norm,idx_importaceTop);
